-- Digital Art NFT Marketplace & Creator Economy - Oracle Object-Relational Database (FINAL WORKING VERSION)
...
-- FIXED: Simplified syntax for substitutable object table
...
-- Create NFT table (simplified without nested tables initially to avoid errors)
CREATE TABLE nft (
...
BEGIN
    ...
    SELECT REF(th) INTO v_transaction_ref 
    FROM transaction_history th 
    WHERE th.transaction_id = v_transaction_id;
...
-- THE 5 REQUIRED COMPLEX QUERIES (SIMPLIFIED FOR CLARITY):

-- Query 1: JOIN of three or more tables with multiple join types and restrictions
-- Shows NFT marketplace data with creator verification status and collection performance
SELECT 
    n.nft_id,
...
-- Gets all wallet addresses that are either NFT creators or current NFT owners
...
-- Shows NFT details with creator type-specific information and nested collection data
SELECT 
    n.nft_id,
    n.title,
    n.price,
    ...
    CARDINALITY(n.transactions) AS total_transactions
FROM nft n, creator c
WHERE n.creator = REF(c)
ORDER BY CARDINALITY(n.transactions) DESC;

-- Query 4: Query using temporal features with intervals and timestamps
-- Analyzes NFT market activity over different time periods
SELECT 
    time_period,
...
-- Test object method functionality
DECLARE
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('Creator Status: ' || v_creator.get_creator_status());
    DBMS_OUTPUT.PUT_LINE('Reputation Score: ' || v_creator.get_reputation_score());
    
    -- Test verified creator methods
...
-- Summary of what this script demonstrates:
-- 1. Object-relational inheritance hierarchy (creator_type -> verified_creator_type -> premium_creator_type)
-- 2. Member functions and procedures with method overriding
-- 3. Object tables with substitutable types
-- 4. REF columns with scoped references
-- 5. Nested tables (transactions, bids)
-- 6. VARRAY types (tag_array_type)
-- 7. Complex queries using DEREF, VALUE, TREAT, IS OF operators
-- 8. Temporal queries with INTERVAL operations
-- 9. OLAP queries with ROLLUP and CUBE
-- 10. Triggers on object tables
-- 11. Procedures and functions working with object types

DBMS_OUTPUT.PUT_LINE('NFT Marketplace Database Schema created successfully!');
DBMS_OUTPUT.PUT_LINE('All object-relational features demonstrated:');
DBMS_OUTPUT.PUT_LINE('- Type inheritance hierarchy');
DBMS_OUTPUT.PUT_LINE('- Member functions and procedures');
DBMS_OUTPUT.PUT_LINE('- Object tables with substitutable types');
DBMS_OUTPUT.PUT_LINE('- REF columns and nested tables');
DBMS_OUTPUT.PUT_LINE('- Complex queries with object operators');
DBMS_OUTPUT.PUT_LINE('- OLAP operations and temporal queries');