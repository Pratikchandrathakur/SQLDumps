-- Digital Art NFT Marketplace & Creator Economy - Oracle Object-Relational Database (FULLY CORRECTED)
...
-- Verified Creator subtype (ALSO NOT FINAL to allow premium_creator_type)
...
-- FIXED: Correct syntax for substitutable object table (remove comma before OBJECT IDENTIFIER)
CREATE TABLE creator OF creator_type (
    PRIMARY KEY (creator_id),
    CONSTRAINT unique_wallet UNIQUE (wallet_address),
    CONSTRAINT unique_username UNIQUE (username)
) SUBSTITUTABLE AT ALL LEVELS OBJECT IDENTIFIER IS SYSTEM GENERATED;
...
-- Create NFT table with proper scoping (now that creator table exists)
CREATE TABLE nft (
...
-- Procedure to process a sale (FIXED: Corrected RETURNING clause)
...
BEGIN
    ...
    -- Get the reference to the transaction just inserted
...
-- Sample NFT creation using the procedure
DECLARE
    v_nft_id NUMBER;
BEGIN
    create_nft(
...
LEFT OUTER JOIN transaction_history t ON t.transaction_id = (
    ...
    FROM transaction_history th
    WHERE th.transaction_type_name = 'SALE'
)
