...
SELECT 
    ...
    AVG(VALUE(t).get_total_cost()) AS avg_total_cost_with_fees,
    MAX(VALUE(t).get_transaction_age()) AS oldest_transaction_age
FROM (
    SELECT 
        VALUE(t) AS t,
        t.amount,
...
DBMS_OUTPUT.PUT_LINE('4. VARRAY types (tag_array_type) for collections');
...
DBMS_OUTPUT.PUT_LINE('=== Database ready for use! ===');